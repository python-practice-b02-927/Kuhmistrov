...
class Target(Agent):
    def __init__(
            self, canvas, x=None, y=None, r=None, vy=None, vx=None,
            color=None, job_init=None):
        super().__init__()
        self.job = job_init

        x = self.x = rnd(350, 850) if x is None else x
        y = self.y = rnd(150, 350) if y is None else y
        r = self.r = rnd(2, 50) if r is None else r
        vy = self.vy = rnd(-10, 10) if vy is None else vy
        vx = self.vx = rnd(-5, 5) if vx is None else vx
...
class BattleField(tk.Canvas):
    def __init__(self, master):
        ...
        self.gun_right = Gun(self, [1150, 450])
...
class MainFrame(tk.Frame):
    ...
    def report_hit(self, bullet, target):
        if self.battlefield.active_gun == 'left':
            self.left_score += 1
            self.left_score_label['text'] = self.score_tmpl.format(self.left_score)
        if self.battlefield.active_gun == 'right':
            self.right_score += 1
            self.right_score_label['text'] = self.score_tmpl.format(self.right_score)
